...
// path, format) is a CLI argument. No logic branches on a specific
// workId, title, author, or book slug. Provider-specific download
// behavior is applied once per provider class before fetching.
//
...
// Standard Ebooks' human-facing /downloads/<file>.epub URL returns a
// thank-you XHTML page on a first anonymous request. Their server's
// own download handler explicitly skips that page when source=download
// (or source=feed) is present and serves the real EPUB bytes instead.
// This is provider-level ingestion behavior: every Standard Ebooks
// record goes through the same rule; no title/work-specific code.
function resolveProviderDownloadUrl(sourceId: string, sourceUrl: string): string {
  if (sourceId !== "standard-ebooks") return sourceUrl;
  ...
  if (parsed.hostname === "standardebooks.org" || parsed.hostname === "www.standardebooks.org") {
    parsed.searchParams.set("source", "download");
...
async function main(): Promise<void> {
  ...
  const downloadUrl = resolveProviderDownloadUrl(args.sourceId, fileEntry.url);

  console.log(`ingestBook: sourceId=${args.sourceId} externalId=${args.externalId} format=${fileEntry.format}`);
  console.log(`ingestBook: sourceUrl=${fileEntry.url}`);
  console.log(`ingestBook: fetching ${downloadUrl}`);

  const fetched = await fetchAndValidate(downloadUrl, fileEntry.format);
