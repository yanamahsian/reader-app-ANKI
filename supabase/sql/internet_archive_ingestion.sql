-- Internet Archive ingestion support — already applied to production 2026-08-29.
--
-- The Edge Function source lives at:
--   supabase/functions/omnia-internet-archive-ingest-one/index.ts
--
-- This file mirrors the production source registration, bounded dispatcher support,
-- and the two curated Nobel-recovery candidates. Internet Archive is NOT a blanket
-- rights authority: the importer stores plaintext OCR plus normalized anki-json and
-- leaves DE rights unknown until the existing deterministic jurisdiction backfill runs.

insert into public.sources (id,name,base_url,trust_level)
values ('internet-archive','Internet Archive','https://archive.org','institutional')
...
select m.id,'internet-archive','einfhrungineine00euckgoog','Einführung in eine Philosophie des Geisteslebens','de',0,'discovered',jsonb_build_object(
  'expectedTitle','Einführung in eine Philosophie des Geisteslebens',
  'expectedCreator','Rudolf Eucken',
  'selection_reason','Nobel recovery: original-language 1908 Internet Archive scan/OCR',
  'openLibraryEvidence','OL175550M'
)
from public.master_corpus_authors m where m.canonical_author_id='rudolf-eucken'
on conflict (source_id,external_id) do nothing;

insert into public.master_corpus_candidates
(master_author_id,source_id,external_id,title,language,ordinal,status,provider_metadata)
select m.id,'internet-archive','imagospitt00spituoft','Imago','de',0,'discovered',jsonb_build_object(
  'expectedTitle','Imago',
  'expectedCreator','Carl Spitteler',
  'selection_reason','Nobel recovery: original-language Internet Archive scan/OCR; 1919 edition of 1906 novel',
  'openLibraryEvidence','OL24436689M'
)
from public.master_corpus_authors m where m.canonical_author_id='carl-spitteler'
on conflict (source_id,external_id) do nothing;

-- Explicit legal hold requested for the Pulitzer recovery queue. Do not ingest before 2034.
update public.master_corpus_authors
set status='blocked',
    notes=case
      when coalesce(notes,'') ilike '%rights-blocked until 2034%' then notes
      when coalesce(notes,'')='' then 'rights-blocked until 2034'
      else notes || '; rights-blocked until 2034'
    end,
    updated_at=now()
where canonical_author_id='oliver-la-farge';
